...
/***
 *** these routines and file roughly correspond to the dm2k.c file and
 ***  the normal (non-embedded) DM2K main() entry point
 ***/
 ...
 *   have limited scope (main(), libDm2kInit() and dmTerminateX() are only
 *   routines which care
 */
Atom DM2K_EDIT_FIXED = (Atom)NULL, DM2K_EXEC_FIXED = (Atom)NULL,
	   DM2K_EDIT_SCALABLE = (Atom)NULL, DM2K_EXEC_SCALABLE = (Atom)NULL;
...
/* need privateData local copy of this string to be passed to dm2kInit() */
  /* T. Straumann: protect against null pointer */
  strncpy(displayFont,fontString ? fontString : "",FONT_NAME_SIZE-1);
...
/*
 * intialize DM2K stuff
 */
  dm2kInit(displayFont);
